  ...
  try {
    ...
    const inventoryWithPnL = await Promise.all(
      inventory.map(async (item) => {
        ...
        const totalBought = transactions.filter(t => t.type === "buy").reduce((sum, t) => sum + t.volume * t.price, 0);
        const totalSold = transactions.filter(t => t.type === "sell").reduce((sum, t) => sum + t.volume * t.price, 0);
        const volumeBought = transactions.filter(t => t.type === "buy").reduce((sum, t) => sum + t.volume, 0);
        const volumeSold = transactions.filter(t => t.type === "sell").reduce((sum, t) => sum + t.volume, 0);
        ...
        return { ...item, currentVolume, averageCost, currentValue, realizedPnL, unrealizedPnL: 0, totalPnL: realizedPnL };
      })
    );
...
export async function createTransaction(req: AuthRequest, res: Response) {
  try {
    ...
    if (inventoryItem.userId !== userId) return res.status(403).json({ success: false, message: "Forbidden" });
    ...
    if (type === "buy") {
      ...
      const newAvgCost = ((inventoryItem.averageCost * inventoryItem.volumeLitres) + (ensureNumber(price) * ensureNumber(volume))) / newVolume;
      await storage.updateInventory(inventoryId, { volumeLitres: newVolume, averageCost: newAvgCost });
    } else if (type === "sell") {
      const newVolume = inventoryItem.volumeLitres - ensureNumber(volume);
      await storage.updateInventory(inventoryId, { volumeLitres: newVolume });
    }
