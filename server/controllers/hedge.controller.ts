...
export async function generateHedgeRecommendations(req: AuthRequest, res: Response) {
  try {
    ...
    depotPrices.forEach(price => {
      if (!pricesByProduct[price.productType]) pricesByProduct[price.productType] = [];
...
export async function getAdvancedAnalysis(req: AuthRequest, res: Response) {
  try {
    ...
    const portfolioValue = inventory.reduce((sum, item) => sum + (item.volumeLitres * item.averageCost), 0);
    ...
    const supplyConstraints = terminalSignals
      ...
      .map(s => ({ terminal: s.terminal.name, supply: s.signal?.nnpcSupply }));
...
function calculateHedgeRatio(volatility: Record<string, number>, spreads: any): number {
  const avgVol = Object.values(volatility).reduce((a, b) => a + b, 0) / Object.keys(volatility).length;
...
function generateSuggestedActions(volatility: Record<string, number>, spreads: any, constraints: any[]): string[] {
  const actions = [];
  if (volatility.PMS > 0.1) actions.push("Consider forward contracts for PMS due to high volatility");
  if (Math.abs(spreads.PMS_AGO) > 40) actions.push(`Spread trade opportunity: ${spreads.PMS_AGO > 0 ? 'AGO over PMS' : 'PMS over AGO'}`);
  if (constraints.length > 0) actions.push(`Build inventory at ${constraints.length} terminals with supply constraints`);
  if (actions.length === 0) actions.push("Market conditions stable. Maintain current hedge positions.");
  return actions;
}
