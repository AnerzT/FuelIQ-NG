...
export async function seedPrismaDatabase() {
  ...
  try {
    ...
    const { rows } = await client.query("SELECT COUNT(*) as count FROM prisma_terminals");
    if (parseInt(rows[0].count) > 0) {
      console.log("Prisma database already seeded, skipping.");
      return;
    }
    ...
    for (const t of TERMINAL_SEEDS) {
      const termResult = await client.query(
        "INSERT INTO prisma_terminals (id, name, state, code, active) VALUES (gen_random_uuid(), $1, $2, $3, true) RETURNING id",
        [t.name, t.state, t.code]
      );
      const terminalId = termResult.rows[0].id;

      const s = SIGNAL_PRESETS[t.code];
      if (s) {
        await client.query(
          "INSERT INTO prisma_market_signals (id, terminal_id, vessel_activity, truck_queue, nnpc_supply, fx_pressure, policy_risk, created_at) VALUES (gen_random_uuid(), $1, $2, $3, $4, $5, $6, NOW())",
          [terminalId, s.vesselActivity, s.truckQueue, s.nnpcSupply, s.fxPressure, s.policyRisk]
        );
      }

      const f = FORECAST_PRESETS[t.code];
      if (f) {
        await client.query(
          "INSERT INTO prisma_forecasts (id, terminal_id, expected_min, expected_max, bias, confidence, suggested_action, created_at) VALUES (gen_random_uuid(), $1, $2, $3, $4, $5, $6, NOW())",
          [terminalId, f.expectedMin, f.expectedMax, f.bias, f.confidence, f.suggestedAction]
        );
      }

      const basePrice = f?.expectedMin || 620;
      const now = new Date();
      for (let i = 30; i >= 0; i--) {
        ...
        await client.query(
          "INSERT INTO prisma_price_history (id, terminal_id, date, price) VALUES (gen_random_uuid(), $1, $2, $3)",
          [terminalId, date, price]
        );
      }
    }

    console.log("Prisma database seeded successfully with 8 terminals.");
  } catch (err) {
    console.log("Prisma seed error (non-fatal):", (err as Error).message);
  } finally {
    await client.end();
  }
}
